...
router.post("/createDeck", validateAuthToken, async (req, res)=>{
    service.createDeck(req.body, res).then(data => {
        res.status(200).send(data)
    })
})

router.post("/getOrCreateDeck", validateAuthToken, async (req, res)=>{
    service.getOrCreateDeck(req.body, res).then(data => {
        res.status(200).send(data)
    })
})
